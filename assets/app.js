...
window.YW = (function(){
  ...
  const SOCIAL = {
    ...
    rednote:{name:"Rednote", bg:"#ff2e47", svg:'<svg viewBox="0 0 24 24"><path d="M12 5.4C10.4 4.1 8 3.4 5.6 3.7c-.9.1-1.6.8-1.6 1.7v11.8c0 .9.8 1.6 1.7 1.5 2.2-.2 4.5.4 6.3 1.6 1.8-1.2 4.1-1.8 6.3-1.6.9.1 1.7-.6 1.7-1.5V5.4c0-.9-.7-1.6-1.6-1.7C16 3.4 13.6 4.1 12 5.4Zm0 0v12.9" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>'},
  ...
  function renderSocial(){
    ...
    fetchData().then(function(j){
      ...
      host.innerHTML=list.filter(function(s){return s.url && SOCIAL[String(s.platform||"").toLowerCase()];}).map(function(s){
        ...
        return '<a href="'+esc(s.url)+'" target="_blank" rel="noopener" aria-label="'+esc(p.name)+'" style="background:'+p.bg+'">'+p.svg+'</a>';
      }).join("");
    }).catch(function(){});
  }
